...
export function createScaleUpService({readSource,readValidation,applyHandoff,filename,intervalMs=60000,now=()=>new Date().toISOString()}={}){
 ...
 router.use((req,res,next)=>{res.set('Cache-Control','no-store');const origin=req.get('origin');if(origin){try{const url=new URL(origin),local=['127.0.0.1','localhost','[::1]'].includes(url.hostname),sameOrigin=url.host===req.get('host');if(!local&&!sameOrigin)return res.status(403).json({error:'Cross-origin requests are not allowed.'});}catch{return res.status(403).json({error:'Invalid origin.'});}}next();});
